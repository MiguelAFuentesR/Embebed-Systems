...
//Credenciales de la red WiFi a crear:
const char* ssid_AP = "WiFi-ESP32";
const char* password_AP = "Embebidos1";
...
void Configuracion_AP() {
  Serial.println("Estableciendo el AP (Access Point)...");
  WiFi.softAP(ssid_AP, password_AP); //Crear AP con ssid y contraseña establecidos previamente
  Serial.println("AP " + String(ssid_AP) + " creado Correctamente!!!");
      ...
      if (client.available()) { //Si hay bytes para leer del cliente...
        ...
        if (c == '\n') { //Si dato es final de Enter, implica final de la petición HTTP del cliente y se debe enviar respuesta
          if (dataIn.length() == 0) {
            ...
            client.println("<body><h1><font color=#FF00FF>ESP32 AP Web Server</font></h1>"); //Titulo de la página Web
